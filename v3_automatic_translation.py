...
class auto_translator:
    ...
    def __init__(self, **kwargs):
        ...
        self.replace_dict = kwargs['replace_dict'] if len(kwargs['replace_dict']) != 0 else None
        self.replace_flag = True if type(self.replace_dict) == dict else False
    ...
    def _keyword_replace(self, sentence):
        
        """self.replace_dictで指定された単語変換を行う。2重変換が起きないようにマッチしたら直接変換するのでなく、
        辞書内の順番を示すインデックス値に1度変換してからstr.format()メソッドを使用して一斉に変換する。"""
        
        index_list = []
        for i, item in enumerate(self.replace_dict.items()):
    ...
    def run_multiThread(self):
        ...
        split_list = self._split_sentences(sentences=self.eg_sentence)
...
#     translator = auto_translator(**kwargs)
#     translator.run_multiThread().saveFile()
    # sentences = sentence_from_docx(**kwargs)
    # translated_texts = run_multiThread(sentences=sentences, **kwargs)
    # saveFile(sentences, translated_texts, **kwargs)
...
if __name__ == "__main__":
    ...
    kwargs = {"Fpath": r"E:\研究関連\文献リスト\LEM", # 要約対象のwordファイルが格納されているディレクトリ
              "Fname": r"sample.docx", # wordファイル名（拡張子込みで指定する）
              "max_worker": 4, # 同時実行スレッド数。各PC事に最大値が違うので要確認。例外が発生してもデッドロックになってしまい特定場合は1に変更する
