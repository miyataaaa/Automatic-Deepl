...
if __name__ == "__main__":
    ...
    kwargs = {"Fpath": r"Z:\miyata\論文", # 要約対象のwordファイルが格納されているディレクトリ
              "Fname": r"Tofelde et al 2022.docx", # wordファイル名（拡張子込みで指定する）
              "max_worker": 1, # 同時実行スレッド数。もし１つのスレッドで例外が発生した場合、デッドロックになってしまいエラーが伝搬してこないので、デバックする際は1スレッドに変更する
